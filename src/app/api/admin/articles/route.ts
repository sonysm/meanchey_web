...
const extractImageName = (rawImage: unknown): string | null => {
    ...
    if (rawImage.startsWith("/")) {
        const file = rawImage.split("/").filter(Boolean).pop();
        return file ?? rawImage;
    }
...
const deltaToDetailPayload = (delta: DeltaPayload): unknown[] => {
    ...
    return ops.map((op, index) => {
        ...
        if (typeof insert === "string") {
            item.text = isRecord(op.attributes)
                ? {
                    insert,
...
const toDetailPayload = (content: string): unknown => {
    ...
    try {
        ...
        return [{ order_no: 0, text: { insert: String(content) }, imgs: [], audio: "", vdo: "", file: null, title: null }];
    } catch {
        return [{ order_no: 0, text: { insert: content }, imgs: [], audio: "", vdo: "", file: null, title: null }];
    }
};
...
export async function POST(request: NextRequest) {
    ...
    if (selectedCompanyId <= 0) {
        return NextResponse.json({ message: "Host company is required" }, { status: 400 });
    }

    if (!allowedCompanyIds.has(selectedCompanyId)) {
        return NextResponse.json(
    ...
    // Keep create payload identical to meanchey_app's aritcleCreate contract.
    const payload = {
        country_code: "kh",
    ...
    const url = new URL("/article-ccreate", API_BASE_URL);
    ...
    if (!response.ok) {
        return NextResponse.json(
            { message: extractMessage(data, `Failed to create article (${response.status})`), data, status: response.status },
